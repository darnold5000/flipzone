...
export function Testimonials() {
  return (
    <section className="section-padding bg-flip-blue/5">
      <div className="container-wide">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {testimonials.map((t, i) => (
            ...
            >
              <Quote className="absolute right-6 top-6 size-10 text-flip-aqua/30" />
              <div className="mb-4 flex gap-1">
                {Array.from({ length: t.rating }).map((_, j) => (
                  <Star key={j} className="size-5 fill-flip-aqua text-flip-aqua" />
                ))}
              </div>
              <p className="text-lg leading-relaxed text-foreground">&ldquo;{t.quote}&rdquo;</p>
              <footer className="mt-6 border-t border-border pt-4">
                <cite className="not-italic">
                  <span className="font-heading font-bold text-flip-blue">{t.author}</span>
